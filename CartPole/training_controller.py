...
from CartPole.memory_manager import MemoryManager
...
def a3c_helper(inp_dict):
    ...
    train(agent, memory_manager, optimizer, device, env, params)

    return agent.model.state_dict(), {"reward": memory_manager.rewards[0], "value_loss": memory_manager.value_loss[0], "loss": memory_manager.loss[0]}
...
def CartPole_train(state_dict, steps, learning_rate, entropy_weight, opponents, devices):
    rewards = []
    places = []
    value_loss = []
    loss = []
    ...
    with mp.Pool(len(devices)) as pool:
        for i in range(steps):
            ...
            print(f"\rCurrent Iteration {i}/{steps}, {100 * i/steps:.2f}%, Value loss: {value_loss[-1]:.2f}", end="")
